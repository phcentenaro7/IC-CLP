...
function as_3Dview(db::Database, node::ContainerNode; colors=cgrad(:rainbow, nrow(db.items), categorical=true), n=Inf)
    drawables = Vector{GenericTrace}()
    meshes = Vector{GenericTrace}()
    outlines = Vector{GenericTrace}()
    placements = filter(row -> row[:status] == :filled, node.spaces)
    for placement in eachrow(placements)
        x, y, z = collect(placement[[:x, :y, :z]])
        w, d, h = collect(placement[[:width, :height, :depth]]) 
        m = create_box_mesh((x, z, y), (w, h, d), colors[placement[:item]] |> rgb_as_string)
        m.name = "($(placement[:item]), $(placement[:id]))"
        push!(meshes, m)
